...
1. CLI 설치: `npm install -g @raeseoklee/mcp-workbench`
...
| 증상 | 원인 | 해결 방법 |
|------|------|-----------|
| `Failed to launch mcp-workbench` | CLI가 PATH에 없음 | `mcpWorkbench.cliPath` 설정 또는 `npm install -g @raeseoklee/mcp-workbench` 실행 |
| `Failed to parse mcp-workbench output` | CLI 버전이 너무 오래됨 | 업그레이드: `npm install -g @raeseoklee/mcp-workbench` |
| `mcp-workbench` 설치했는데 다른 도구가 실행됨 | 관련 없는 npm 패키지 | 해당 패키지 제거 후 `@raeseoklee/mcp-workbench` 또는 `mcp-workbench-cli` 설치 |
...
npm의 unscoped 패키지 `mcp-workbench`는 관련 없는 별개 프로젝트(MCP 서버 aggregator)가 사용 중입니다. 저희 CLI는 `@raeseoklee/mcp-workbench`로 배포되며, `mcp-workbench-cli`는 편의를 위한 대안입니다. 두 가지 모두 동일한 `mcp-workbench` 커맨드를 설치합니다.
